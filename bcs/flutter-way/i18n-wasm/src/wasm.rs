...
impl WasmCompiledTemplate {
    #[wasm_bindgen]
    pub fn render(&self, args_js: JsValue) -> String {
        let args: Option<HashMap<String, String>> = if args_js.is_null() || args_js.is_undefined() {
            None
        } else {
            serde_wasm_bindgen::from_value(args_js).ok()
        };
