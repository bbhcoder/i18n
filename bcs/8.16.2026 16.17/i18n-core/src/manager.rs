...
pub struct I18nManager {
    ...
    pub media_store: HashMap<String, HashMap<String, HashMap<String, String>>>,
    pub debug_mode: bool,
}
...
impl I18nManager {
    ...
    pub fn diagnostics(&self) -> String {
        let mut report = String::from("=== I18n SDK Diagnostics ===\n");
        report.push_str(&format!("Current Locale: {}\n", self.current_locale));
        report.push_str(&format!("Fallback Locale: {}\n", self.fallback_locale));
        
        report.push_str("\nLoaded Keys (Flat Memory):\n");
        report.push_str(&format!("Total Keys in Memory: {}\n\n", self.store.len()));
        
        // گروه‌بندی کلیدها برای نمایش بهتر در گزارش
        let mut ns_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
        for k in self.store.keys() {
            // کلید ما به این شکله: locale:namespace:key
            let parts: Vec<&str> = k.splitn(3, ':').collect();
            if parts.len() == 3 {
                let loc_ns = format!("Locale [{}] -> Namespace '{}'", parts[0], parts[1]);
                *ns_counts.entry(loc_ns).or_insert(0) += 1;
            }
        }
        
        for (loc_ns, count) in ns_counts {
    ...
    pub fn add_media(&mut self, locale: &str, namespace: &str, key: &str, url: &str) {
        self.media_store
    ...
    pub fn get_media(&self, namespace: &str, key: &str) -> Option<String> {
        if let Some(url) = self.media_store
    ...
    fn lookup(&self, locale: &str, namespace: &str, key: &str) -> Option<&Arc<Vec<Token>>> {
        let composite_key = format!("{}:{}:{}", locale, namespace, key);
        self.store.get(&composite_key)
    }
    ...
    pub fn t(
        ...
        if self.debug_mode {
            eprintln!(
                "[I18N VALIDATOR WARNING]: Translation missing for '{}.{}' in locale '{}'",
                namespace, key, self.current_locale
            );
        }
    ...
    // متد راهنمای تعاملی برای توسعه‌دهندگان
    pub fn help(&self) -> String {
        let help_text = r#"
=========================================================
🌍 i18n-sdk Interactive Tour & Quick Reference 🌍
=========================================================

Welcome to the blazing fast i18n engine! Here are the core methods:

1. Translation (t)
   engine.t("namespace", "key", { varName: "Value" });
   Example: engine.t("home", "welcome", { name: "Arsalan" });

2. Changing Locales
   engine.setLocale("fa");
   engine.setFallbackLocale("en");

3. Adding Translations (JSON Bundle)
   engine.addBundle("en", "home", '{"welcome": "Hello {name}"}');

4. Media Management (Async) - *Node.js only*
   await engine.scanMediaFolderAsync("assets", "./public/media", "/media");
   let url = engine.getMedia("assets", "logo");

5. Diagnostics & Debugging
   engine.setDebugMode(true); // Shows warnings for missing keys
   console.log(engine.diagnostics()); // Shows memory state
